...
def test_stratified_split_sizes(mock_df):
    ...
    split = stratified_split(mock_df, valid_size=0.2, oot_size=0.2,
                             random_state=42)
    total = len(split.y_train) + len(split.y_valid) + len(split.y_test)
    assert total == len(mock_df)
    assert abs(len(split.y_test) / len(mock_df) - 0.2) < 0.01
...
def test_stratified_split_preserves_bad_rate(mock_df):
    ...
    rates = [split.y_train.mean(), split.y_valid.mean(), split.y_test.mean()]
...
def test_stratified_split_no_overlap(mock_df):
    ...
    test_ids = set(split.X_test["sk_id_curr"])
